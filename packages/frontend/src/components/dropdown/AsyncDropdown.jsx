...
const fetchList = async ({ name, componentData }) => {
  const loadMethod = componentData.inputParams.find(
  ...
  let lists = await axios
    .post(
      `${baseURL}/api/v1/load-methods/${componentData.name}`,
      { ...componentData, loadMethod },
...
export const AsyncDropdown = ({
  ...
  const fetchCredentialList = async () => {
    try {
      ...
      const resp = await credentialsApi.getCredentialsByNames(names);
      if (resp.data) {
        const returnList = [];
  ...
  const fetchOptions = async () => {
    ...
    if (credentialNames.length) {
      ...
      fetched = await fetchList({ name, componentData });
    }
