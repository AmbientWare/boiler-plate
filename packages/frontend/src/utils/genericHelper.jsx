...
export const getUpsertDetails = (nodes, edges) => {
  ...
  for (const edge of edges) {
    if (vsNodeIds.includes(edge.source) || vsNodeIds.includes(edge.target)) {
      ...
      if (vsNode.data.inputs.document)
        connectedDocs = [...new Set(vsNode.data.inputs.document)];

      if (connectedDocs.length) {
        const innerNodes = [vsNode];

        if (vsNode.data.inputs.embeddings) {
          const embeddingsId = vsNode.data.inputs.embeddings
            .replace(/{{|}}/g, "")
            .split(".")[0];
        ...
        for (const doc of connectedDocs) {
          ...
          if (docNode && docNode.data.inputs.textSplitter) {
            const textSplitterId = docNode.data.inputs.textSplitter
              .replace(/{{|}}/g, "")
              .split(".")[0];
...
export const getConfigExamplesForJS = (
  configData,
  bodyType,
  isMultiple,
  stopNodeId,
...
export const getConfigExamplesForPython = (
  configData,
  bodyType,
  isMultiple,
  stopNodeId,
...
export const getConfigExamplesForCurl = (
  configData,
  bodyType,
  isMultiple,
  stopNodeId,
  ...
  for (let i = 0; i < loop; i += 1) {
    ...
    if (config.type === "string")
      ...
      exampleVal = `@/home/user1/Desktop/example${
        config.type.includes(",") ? config.type.split(",")[0] : config.type
      }`;
    finalStr +=
      bodyType === "json"
        ? `"${config.name}": ${exampleVal}`
...
export const validateNode = (node) => {
  ...
  node.data.inputAnchors.map((inputAnchor) => {
    if (inputAnchor.optional || inputAnchor.default) return null;
    if (node.data.inputs[inputAnchor.name]) return null;
    unconfigured.push(inputAnchor);
    return null;
  });
  return unconfigured;
};
